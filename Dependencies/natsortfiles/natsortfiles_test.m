...
chk(Aa, fnh, @i, [3,1,2], {{'a',2;'a',10;'a',1},{'.txt';'.txt';'.txt'}}) % not in Mfile
%
...
chk(Ab, fnh, @i, [3;5;1;4;2], {{'test',2,[];'test',10,'-old';'test',[],[];'test',10,[];'test',1,[]},{'.m';'.m';'.m';'.m';'.m'}}) % not in Mfile
%
...
chk(Ac, fnh, @i, [4;5;3;1;2], {{'A',2,'-old';'A',10,[];'A',2,[];'A',1,[];'A',1,'-archive'},{'test';'test';'test';'test';[]},{'.m';'.m';'.m';'.m';'.zip'}}) % not in Mfile
%
Ad =         {'A1\B','A+/B','A/B1','A=/B','A\B0'};
chk(Ad, fnh, {'A\B0','A/B1','A1\B','A+/B','A=/B'})
chk(Ad, fnh, {'A\B0','A/B1','A1\B','A+/B','A=/B'}, [5,3,1,2,4]) % not in Mfile
chk(Ad, fnh, @i, [5,3,1,2,4], {{'A',1;'A+',[];'A',[];'A=',[];'A',[]},{'B',[];'B',[];'B',1;'B',[];'B',0},cell(5,0)}) % not in Mfile
%
...
chk(Af, fnh, @i, [3;2;1], {{'test_new';'test-old';'test'},{'.m';'.m';'.m'}}) % not in Mfile
%
%% HTML Examples %%
%
...
chk(Ab,              fnh, txf({'1.2.txt','1.3.txt','1.10.txt'}))
chk(Ab,              fnh, txf({'1.2.txt','1.3.txt','1.10.txt'}), [3,1,2]) % not in HTML
chk(Ab, '\d+\.?\d*', fnh, txf({'1.10.txt','1.2.txt','1.3.txt'}))
chk(Ab, '\d+\.?\d*', fnh, txf({'1.10.txt','1.2.txt','1.3.txt'}), [2,3,1]) % not in HTML
chk(Ab,              fnh, @i, [3,1,2], {{1,'.',3;1,'.',10;1,'.',2},{'.txt';'.txt';'.txt'}}) % not in HTML
chk(Ab, '\d+\.?\d*', fnh, @i, [2,3,1], {{1.3;1.1;1.2},{'.txt';'.txt';'.txt'}}) % not in HTML
%
chk({'natsort_doc.html','natsortrows_doc.html','..','.'}, [], 'rmdot', fnh, ...
	{'natsort_doc.html','natsortrows_doc.html'})
%
Ac =                               txf({'1.9','1.10','1.2'});
chk(Ac, '\d+\.?\d*',          fnh, txf({'1.2','1.9','1.10'}))
chk(Ac, '\d+\.?\d*',          fnh, txf({'1.2','1.9','1.10'}), [3,1,2]) % not in HTML
chk(Ac, '\d+\.?\d*', 'noext', fnh, txf({'1.10','1.2','1.9'}))
chk(Ac, '\d+\.?\d*', 'noext', fnh, txf({'1.10','1.2','1.9'}), [2,3,1]) % not in HTML
chk(Ac, '\d+\.?\d*',          fnh, @i, [3,1,2], {{1;1;1},{'.',9;'.',10;'.',2}}) % not in HTML
chk(Ac, '\d+\.?\d*', 'noext', fnh, @i, [2,3,1], {{1.9;1.1;1.2}}) % not in HTML
%
Ad =                      txf({'B/3.txt','A/1.txt','B/100.txt','A/20.txt'});
chk(Ad,              fnh, txf({'A/1.txt','A/20.txt','B/3.txt','B/100.txt'}))
chk(Ad,              fnh, txf({'A/1.txt','A/20.txt','B/3.txt','B/100.txt'}), [2,4,1,3]) % not in HTML
chk(Ad, [], 'xpath', fnh, txf({'A/1.txt','B/3.txt','A/20.txt','B/100.txt'}))
chk(Ad, [], 'xpath', fnh, txf({'A/1.txt','B/3.txt','A/20.txt','B/100.txt'}), [2,1,4,3]) % not in HTML
chk(Ad,              fnh, @i, [2,4,1,3], {{'B';'A';'B';'A'},{3;1;100;20},{'.txt';'.txt';'.txt';'.txt'}}) % not in HTML
chk(Ad, [], 'xpath', fnh, @i, [2,1,4,3], {{3;1;100;20},{'.txt';'.txt';'.txt';'.txt'}}) % not in HTML
%
...
chk(Ae, [], fnh, @i, @i, {{'B';10;1;'A';2},{'.txt';'.txt';'.txt';'.txt';'.txt'}}) % not in HTML
%
Bf = {{'abc',2,'xyz';'abc',10,'xyz';'abc',2,'xyz';'abc',1,'xyz'},{'.txt';'.txt';'.txt';'.txt'}};
Af =         txf({'abc2xyz.txt','abc10xyz.txt','abc2xyz.txt','abc1xyz.txt'});
chk(Af, fnh, txf({'abc1xyz.txt','abc2xyz.txt','abc2xyz.txt','abc10xyz.txt'}))
chk(Af, fnh, txf({'abc1xyz.txt','abc2xyz.txt','abc2xyz.txt','abc10xyz.txt'}), [4,1,3,2])  % not in HTML
chk(Af, fnh, txf({'abc1xyz.txt','abc2xyz.txt','abc2xyz.txt','abc10xyz.txt'}), [4,1,3,2], Bf) % not in HTML
chk(Af, fnh, @i, @i, Bf)
%
...
chk(Aj,                    fnh, txf({'1.23V.csv','1.200V.csv','010V.csv','+1.csv','-1V.csv'}))
chk(Aj,                    fnh, txf({'1.23V.csv','1.200V.csv','010V.csv','+1.csv','-1V.csv'}), [1,5,4,3,2]) % not in HTML
chk(Aj, '(+|-)?\d+\.?\d*', fnh, txf({'-1V.csv','+1.csv','1.200V.csv','1.23V.csv','010V.csv'}))
chk(Aj, '(+|-)?\d+\.?\d*', fnh, txf({'-1V.csv','+1.csv','1.200V.csv','1.23V.csv','010V.csv'}), [2,3,5,1,4]) % not in HTML
chk(Aj,                    fnh, @i, [1,5,4,3,2], {{1,'.',23,'V';'-',1,'V',[];'+',1,[],[];10,'V',[],[];1,'.',200,'V'},{'.csv';'.csv';'.csv';'.csv';'.csv'}}) % not in HTML
chk(Aj, '(+|-)?\d+\.?\d*', fnh, @i, [2,3,5,1,4], {{1.23,'V';-1,'V';1,[];10,'V';1.2,'V'},{'.csv';'.csv';'.csv';'.csv';'.csv'}}) % not in HTML
%
Ak =                     txf({'1,3.txt', '1,10.txt', '1,2.txt'});
chk(Ak, '\d+,?\d*', fnh, txf({'1,10.txt', '1,2.txt', '1,3.txt'}))
...
chk(K,                 fnh, {'00','0','1','2','20','100'}, [2,5,4,6,3,1], L)
chk(K, [], 'num<char', fnh, {'00','0','1','2','20','100'}, [2,5,4,6,3,1], L)
chk(K, [], 'char<num', fnh, {'00','0','1','2','20','100'}, [2,5,4,6,3,1], L)
chk(K, [],   'ascend', fnh, {'00','0','1','2','20','100'}, [2,5,4,6,3,1], L)
chk(K, [],  'descend', fnh, {'100','20','2','1','00','0'}, [1,3,6,4,2,5], L)

K =                         {'00','0','000','0','00','0'}; L = {num2cell(str2double(K(:))),cell(6,0)};
chk(K,                 fnh, {'00','0','000','0','00','0'}, [1,2,3,4,5,6], L)
chk(K, [], 'num<char', fnh, {'00','0','000','0','00','0'}, [1,2,3,4,5,6], L)
chk(K, [], 'char<num', fnh, {'00','0','000','0','00','0'}, [1,2,3,4,5,6], L)
chk(K, [],   'ascend', fnh, {'00','0','000','0','00','0'}, [1,2,3,4,5,6], L)
chk(K, [],  'descend', fnh, {'00','0','000','0','00','0'}, [1,2,3,4,5,6], L)
%
K =                         {'BA','B','BAA','B','AA','A','CA','A','C'}; L = {K(:),cell(9,0)};
chk(K,                 fnh, {'A','A','AA','B','B','BA','BAA','C','CA'}, [6,8,5,2,4,1,3,9,7], L)
chk(K, [], 'num<char', fnh, {'A','A','AA','B','B','BA','BAA','C','CA'}, [6,8,5,2,4,1,3,9,7], L)
chk(K, [], 'char<num', fnh, {'A','A','AA','B','B','BA','BAA','C','CA'}, [6,8,5,2,4,1,3,9,7], L)
chk(K, [],   'ascend', fnh, {'A','A','AA','B','B','BA','BAA','C','CA'}, [6,8,5,2,4,1,3,9,7], L)
chk(K, [],  'descend', fnh, {'CA','C','BAA','BA','B','B','AA','A','A'}, [7,9,3,1,2,4,5,6,8], L)
%
%% DIR Structure %%
%
...
%
V =                        {'x';'z';'y';'';'z';'';'x';'y'};
chk(V,                fnh, {'';'';'x';'x';'y';'y';'z';'z'},[4;6;1;7;3;8;2;5])
chk(V, [],  'ascend', fnh, {'';'';'x';'x';'y';'y';'z';'z'},[4;6;1;7;3;8;2;5])
chk(V, [], 'descend', fnh, {'z';'z';'y';'y';'x';'x';'';''},[2;5;3;8;1;7;4;6])
%
W =                        {'2x';'2z';'2y';'2';'2z';'2';'2x';'2y'};
chk(W,                fnh, {'2';'2';'2x';'2x';'2y';'2y';'2z';'2z'},[4;6;1;7;3;8;2;5])
chk(W, [],  'ascend', fnh, {'2';'2';'2x';'2x';'2y';'2y';'2z';'2z'},[4;6;1;7;3;8;2;5])
chk(W, [], 'descend', fnh, {'2z';'2z';'2y';'2y';'2x';'2x';'2';'2'},[2;5;3;8;1;7;4;6])
%
...
%
X =                               {'1.10','1.2'};
chk(X, '\d+\.?\d*',          fnh, {'1.2','1.10'}, [2,1], {{1;1},{'.',10;'.',2}})
chk(X, '\d+\.?\d*', 'noext', fnh, {'1.10','1.2'}, [1,2], {{1.1;1.2}})
%
Y =                                          {'1.2','2.2','20','2','2.10','10','1','2.00','1.10'};
chk(Y, '\d+\.?\d*',                     fnh, {'1','1.2','1.10','2','2.00','2.2','2.10','10','20'},[7,1,9,4,8,2,5,6,3])
chk(Y, '\d+\.?\d*', 'noext',            fnh, {'1','1.10','1.2','2','2.00','2.10','2.2','10','20'},[7,9,1,4,8,5,2,6,3])
chk(Y, '\d+\.?\d*', 'noext',  'ascend', fnh, {'1','1.10','1.2','2','2.00','2.10','2.2','10','20'},[7,9,1,4,8,5,2,6,3])
chk(Y, '\d+\.?\d*', 'noext', 'descend', fnh, {'20','10','2.2','2.10','2','2.00','1.2','1.10','1'},[3,6,2,5,4,8,1,9,7])
%
