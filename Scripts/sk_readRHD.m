...
function intanData = sk_readRHD(filename)
...
if (data_present)
    ...
    board_adc_data = zeros(num_board_adc_channels, num_board_adc_samples,'int32');
    ...
    for i=1:num_data_blocks
        ...
        if (num_amplifier_channels > 0)
            amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16=>int32')';
        end
        ...
        if (num_board_adc_channels > 0)
            board_adc_data(:, board_adc_index:(board_adc_index + num_samples_per_data_block - 1)) = fread(fid, [num_samples_per_data_block, num_board_adc_channels], 'uint16=>int32')';
        end
        if (num_board_dig_in_channels > 0)
...
if (data_present)
    ...
    amplifier_data = int16(amplifier_data - int32(32768)); % units = microvolts/scaleFactor
    ...
    if (board_mode == 1)
        ...
        adcScaleFactor = 152.59e-6;
        board_adc_data = int16(board_adc_data - int32(32768)); % units = volts/adcScaleFactor
    elseif (board_mode == 13) % Intan Recording Controller
        %         board_adc_data = 312.5e-6 * (board_adc_data - 32768); % units = volts
        adcScaleFactor = 312.5e-6;
        board_adc_data = int16(board_adc_data - int32(32768)); % units = volts/adcScaleFactor
    else
