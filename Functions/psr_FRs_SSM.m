...
%%
swdVec = false(size(TS)); % state vector to track state
% -- Find start and end times of seizures -- %
for szi = 1:numel(seizures)
    se(1) = seizures(szi).time(seizures(szi).trTimeInds(1));   % seizure start time
    se(2) = seizures(szi).time(seizures(szi).trTimeInds(end)); % seizure end time
    se = round(se*FS); % put into timestamp units
    xLog = TS >= se(1) & TS <= se(2);
    swdVec(xLog) = true; % SWDs
end

stillnessVec = false(size(TS));% state vector for motion and stillness
for eii = 1:size(spd.stillTimes,1)
    se(1) = spd.stillTimes(eii,1);
    se(2) = spd.stillTimes(eii,2);
    se = round(se*FS); % put into timestamp units
    xLog = TS >= se(1) & TS <= se(2);
    stillnessVec(xLog) = true;
end
