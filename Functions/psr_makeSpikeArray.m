function [spikeArray] = psr_makeSpikeArray(xdir)
...
origDir = cd;
cd(xdir);
clustinfo = tdfread('cluster_info.tsv');
goodLog = strcmp(cellstr(clustinfo.group),'good');
goodClusts = clustinfo.cluster_id(goodLog);
spikeTimes = double(readNPY('spike_times.npy'))/FS;
