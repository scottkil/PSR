function [psth, timevec] = psr_eiRatioPSTH(topdir, twin, dt,buff)
%% psr_eiRatioPSTH Generates peri-SWD time histogram for proportion inhibitory neuronal activity
%
% INPUTS:
...
%           - aa: start PSTHs. Values are proportion inhibitory spikes (i.e. how many of total spikes are from inhibitory neurons)
%           - bb: end PSTHs. same organization as aa but for event ENDs
...
for brii = 1:numel(eiVec.sn) % brain region (structure) loop
    ...
    sumSpikes = eiVec.vals{brii}(1,:)+eiVec.vals{brii}(2,:);
    propI = eiVec.vals{brii}(2,:)./sumSpikes;
    stdprop = std(propI,'omitmissing'); 
    meanprop = mean(propI,'omitmissing');
    zI = (propI-meanprop)/stdprop ;
    zI = propI;
    ...
    psth(brii).aa = mean(AAi,1,'omitmissing');
    psth(brii).bb = mean(BBi,1,'omitmissing');
