...
%       totspks: total number of spikes that go into phase analysis
...
for ni = 1:size(szCounts{1},1)
    ...
    mvac = rad2deg(angle(mean_vector));              % convert mean vector angle to degrees
    ...
    set(cph,'FaceAlpha',0.85,'FaceColor',colorList(ni,:),'EdgeAlpha',1,'EdgeColor','none');

    strng1 = sprintf('Mean Vector Length:  %.2f\nMean Vector Angle:   %.2f^{\\circ}',...
        mv.L(ni),mv.a(ni));
    annotation('textbox', [0.10, 0.7, 0.4, 0.25], ...  % [x y width height] in normalized units
        'String', strng1, ...
        'EdgeColor', 'none', ...
        'Color', 'k',...
        'FontSize',14);
